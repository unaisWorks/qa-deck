...
import { signUpWithEmail, handleGoogleRedirect, getAuthErrorMessage } from "@/lib/auth";
      ...
      <div className="w-full max-w-md">
        <div className="bg-bg-card border border-border rounded-2xl p-8">
          <div className="mb-8">
            ...
            <p className="text-white/50 text-sm">Start generating tests in minutes — free forever</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" autoComplete="name" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Smith"
            ...
            <div>
              ...
              <input type="password" autoComplete="new-password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimum 8 characters"
                className="w-full bg-bg-elevated border border-border rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 focus:outline-none focus:border-green/50 transition-colors"/>
            </div>
            ...
            <p className="text-center text-xs text-white/30 mt-2">By signing up you agree to our terms of service.</p>
          </form>
        </div>
        <p className="text-center text-sm text-white/40 mt-5">
          Already have an account?{" "}
          <Link href="/signin" className="text-green hover:text-green-dark transition-colors font-medium">Sign in</Link>
        </p>
      </div>
    </div>
  );
