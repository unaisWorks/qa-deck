...
async function saveSettings() {
  ...
  if (!provider) {
    showToast("Unrecognised key format. Expected sk-ant- (Claude), sk- (OpenAI), AIza (Gemini), xai- (Grok), gsk_ (Groq), or LA- (Meta)", "error");
    return;
  }
...
async function startScan() {
  ...
  if (!state.apiKey) {
    showToast("Add your AI API key in Settings first", "error");
    toggleSettings(true);
    return;
  }
